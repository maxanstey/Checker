...
class TestChecker extends KernelTestCase
{
    /**
     * A list of anagrams grouped in threes. These could be
    ...
    public function testPalindromeChecker(): void
    {
        foreach (self::ANAGRAMS as $anagram) {
            $this->assertFalse(
                $this->checker->isPalindrome($anagram)
            );
        }

        foreach (self::PALINDROMES as $palindrome) {
            $this->assertTrue(
                $this->checker->isPalindrome($palindrome)
            );
        }
    }
    ...
    public function testAnagramChecker(): void
    {
        foreach (self::ANAGRAMS as $index => $anagram) {
            ...
            $this->assertTrue(
                $this->checker->isAnagram($anagram, self::ANAGRAMS[$index + 1])
            );

            $this->assertTrue(
                $this->checker->isAnagram($anagram, self::ANAGRAMS[$index + 2])
            );
        }
        ...
        foreach (self::PALINDROMES as $index => $palindrome) {
            ...
            $this->assertFalse(
                $this->checker->isAnagram($palindrome, self::PALINDROMES[$index + 1])
            );
        }
    }

    public function testPangramChecker(): void
    {
        foreach (self::ANAGRAMS as $anagram) {
            $this->assertFalse(
                $this->checker->isPangram($anagram)
            );
        }

        foreach (self::PANGRAMS as $pangram) {
            $this->assertTrue(
                $this->checker->isPangram($pangram)
            );
        }
    }
}
